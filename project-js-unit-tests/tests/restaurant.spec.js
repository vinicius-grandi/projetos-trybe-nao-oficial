...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const menuKeys = Object.keys(createMenu({ food: {}, drink: {}}));
    const returnedObj = createMenu({food: {}, drink: {}});

    assert.strictEqual(
        (typeof returnedObj.fetchMenu === 'function' && menuKeys.length > 1), true
      );
    ...
    assert.deepStrictEqual(
      createMenu({food: {}, drink: {}}).fetchMenu(), { food: {}, drink: {} } 
    );
    ...
    objetoFinal.order('coxinha');
    assert.strictEqual(objetoFinal.pay(), 11.7);
  });
});
