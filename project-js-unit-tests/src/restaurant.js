...
const createMenu = ({ food, drink }) => {
  ...
  return {
    ...
    pay() {
      const foodNdrink = [...Object.entries(food), ...Object.entries(drink)];
      let bill = 0;
      
      foodNdrink.forEach((val) => {
        if (this.consumption.includes(val[0])) {
          const regex = new RegExp(`${val[0]}`, 'gi');
          const timesOrdered = [
            ...this.consumption.join('/').matchAll(regex),
          ].length;

          bill += timesOrdered * (val[1]);
        }
      });
